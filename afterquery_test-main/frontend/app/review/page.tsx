...
async function fetchReview(inviteId: string): Promise<ReviewData> {
  const res = await fetch(`${API_BASE_URL}/api/review/invite/${inviteId}`, {
    cache: "no-store",
  });
...
export default async function ReviewPage({
  ...
  if (!inviteId) {
    return (
      <main>
        <h2>Review</h2>
        <p>
          Provide an inviteId query param, e.g.{" "}
          <code>/review?inviteId=UUID</code>
        </p>
      </main>
    );
  }
  ...
  const AdminReviewPanel = dynamic(
    () => import("@/components/AdminReviewPanel"),
    { ssr: false }
  );
  return (
    <main>
