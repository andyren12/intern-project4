...
export default function CandidateStartPage({ slug }: { slug: string }) {
  ...
  const [git, setGit] = useState<{ clone_url: string; branch: string } | null>(
    null
  );
  ...
  async function handleSubmit() {
    try {
      ...
      setMeta(m);
      setSuccess(
        "Your assessment has been submitted successfully. The client will review and follow up shortly."
      );
      setGit(null);
      setRepoUrl(null);
      setToken(null);
    ...
    <div className="grid gap-6 max-w-2xl mx-auto p-6">
      ...
      <div className="flex items-center gap-3">
        ...
        {invite.started_at && (
          <span className="text-xs text-gray-500">
            Started on {new Date(invite.started_at).toLocaleString()}
          </span>
        )}
      </div>
                  ...
                  {commits.map((c) => (
                    <li key={c.sha} className="py-2">
                      <div className="flex justify-between flex-wrap gap-2">
                        ...
                        <span className="text-gray-600 truncate text-right max-w-[48%]">
                      ...
                      <div className="flex justify-between items-center gap-2 text-xs mt-0.5">
                        ...
                        <span className="text-gray-400 text-right min-w-[110px]">
      ...
      <div className="flex gap-3">
        <button
          ...
          className={`rounded px-4 py-2 text-white font-semibold transition ${
            invite.status !== "pending"
